...
interface CanvasComponentResult {
...
export function getCanvas2dRender(seed: string): string|null {
  ...
  const hash  = hmac(sha256 as any, seed).update(text).digest('hex')
...
export async function component(seed: string): Promise<CanvasComponentResult> {
  return {
